...
impl DataShard {
    ...
    pub fn read_item_from_index(&self, header_offset_pos: usize) -> Result<Vec<u8>, DataShardErrors> {
    ...
    pub fn read_item(&self, file_offset: u64) -> Result<Vec<u8>, DataShardErrors> {
        ...
        if let Some(pos) = header_read.get_offset_pos(file_offset).ok() {
            ...
            let end_offset = {
                ...
                if end_offset == 0 {
                    let metadata = self.file.read().unwrap().metadata().expect("Failed to get file metadata");
            ...
            file.seek(SeekFrom::Start(start_offset)).expect("Failed to seek to start_offset");
    ...
    pub fn insert_item(&self, item: Vec<u8>) -> Result<(), DataShardErrors> {
        ...
        match file.flush() {
            ...
            Err(_) => {
                Err(DataShardErrors::FlushingError)
            }
        }
    }
...
mod test {
    ...
    pub async fn test_data_shard() {
        ...
        let file_path = temp_dir.path().join(format!("{}.bin", Uuid::new_v4().to_string()));
    ...
    pub async fn test_data_shard_threads() {
        ...
        let thread_1 = std::thread::spawn(move || {
            ref_shard.write().unwrap().insert_item(b"Hello World".to_vec()).unwrap();
        });

        let ref_shard = shard.clone();
        let thread_2 = std::thread::spawn(move || {
            ref_shard.write().unwrap().insert_item(b"Cats are beautiful".to_vec()).unwrap();
        });
